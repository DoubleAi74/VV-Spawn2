...
for (const [prefix, fields] of [['dashboard', DASHBOARD_INFO_FIELDS], ['pageMetaData', PAGE_INFO_FIELDS]]) {
  ...
  test(`${prefix}: column counts are bounded integers and reset is an explicit null`, () => {
    assert.deepEqual(infoFieldSet({ gridCols: 8 }, fields, prefix), { [`${prefix}.gridCols`]: 8 });
    assert.deepEqual(infoFieldSet({ gridCols: null }, fields, prefix), { [`${prefix}.gridCols`]: null });
    for (const value of [0, 9, 1.5, '4', false, {}, undefined]) {
      assert.throws(() => parseInfoPatch({ gridCols: value }, fields), /whole number/);
    }
  });
}
...
test('conditional responses detect a layout-only change', () => {
  const url = 'https://example.test/info';
  const first = infoResponse(new Request(url), { gridCols: 4 });
  const request = new Request(url, { headers: { 'If-None-Match': first.headers.get('etag') } });
  assert.equal(infoResponse(request, { gridCols: 6 }).status, 200);
});
