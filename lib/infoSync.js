import { normalizeInfoValues, parseInfoPatch } from './infoFields.js';
...
export function createInfoSync({
  ...
  const keys = fields.flatMap(({ text, mode }) => [text, mode]);
  let values = normalizeInfoValues(initialValues, fields);
