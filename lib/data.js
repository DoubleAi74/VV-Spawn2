...
import { DASHBOARD_INFO_FIELDS, PAGE_INFO_FIELDS, infoFieldSet } from '@/lib/infoFields';
...
export function toPublicUser(user, { isOwner = false } = {}) {
  ...
  return {
    ...
    dashboard: (() => {
      const infoText = user.dashboard?.infoText || '';
      const infoText1 = user.dashboard?.infoText1 || '';
      return {
        infoText,
        infoMode: normalizeInfoMode(user.dashboard?.infoMode, infoText),
        infoText1,
        infoMode1: normalizeInfoMode(user.dashboard?.infoMode1, infoText1),
      };
    })(),
  };
}
