...
import { DASHBOARD_INFO_FIELDS, PAGE_INFO_FIELDS, infoFieldSet, normalizeInfoValues } from '@/lib/infoFields';
...
/**
 * A tag is unavailable if it is reserved, currently held, **or still
 * redirecting to someone**. Ignoring the history would let a new account claim
 * an address that sends its visitors somewhere else entirely — which is worse
 * than the broken link the history exists to prevent. See LNK-3.
 *
 * `excludeUserId` lets an account reclaim one of its own former tags.
 */
async function usernameTagIsTaken(tag, excludeUserId = null) {
  if (isReservedUsernameTag(tag)) return true;
  const query = { $or: [{ usernameTag: tag }, { previousTags: tag }] };
  if (excludeUserId) query._id = { $ne: excludeUserId };
  return Boolean(await User.findOne(query, { _id: 1 }).lean());
}

async function uniqueUsernameTag(baseTag, excludeUserId = null) {
  await connectDB();
  let tag = baseTag || 'user';
  if (tag.length < 2) tag = tag + '0';
  if (!(await usernameTagIsTaken(tag, excludeUserId))) return tag;
  let suffix = 2;
  while (true) {
    const candidate = `${tag}-${suffix}`;
    if (!(await usernameTagIsTaken(candidate, excludeUserId))) return candidate;
    suffix++;
  }
}

/** The same rule for page slugs: a former address is not free. See LNK-3. */
async function pageSlugIsTaken(userId, slug, excludePageId = null) {
  const query = { userId, $or: [{ slug }, { previousSlugs: slug }] };
  if (excludePageId) query._id = { $ne: excludePageId };
  return Boolean(await Page.findOne(query, { _id: 1 }).lean());
}
...
async function uniquePageSlug(userId, baseSlug, excludePageId = null) {
  ...
  if (!(await pageSlugIsTaken(userId, slug, excludePageId))) return slug;
  let suffix = 2;
  while (true) {
    const candidate = `${slug}-${suffix}`;
    if (!(await pageSlugIsTaken(userId, candidate, excludePageId))) return candidate;
    suffix++;
  }
}
...
export function toPublicUser(user, { isOwner = false } = {}) {
  ...
  return {
    ...
    dashboard: normalizeInfoValues(user.dashboard, DASHBOARD_INFO_FIELDS),
  };
}
...
export async function getUserTheme(tag) {
  ...
  return User.findOne(
    { usernameTag: tag },
    { _id: 0, 'dashboard.dashHex': 1, 'dashboard.backHex': 1, 'dashboard.themeUpdatedAt': 1 }
  ).lean();
}

/**
 * Everything the sitemap may list: every profile, and every page that is not
 * private. Private pages 404 for anyone but their owner, so listing one would
 * be advertising a dead end. Former slugs are deliberately absent — they
 * redirect, and a sitemap of redirects is noise. See LNK-2.
 */
export async function getSitemapEntries() {
  await connectDB();
  const [users, pages] = await Promise.all([
    User.find({}, { usernameTag: 1, createdAt: 1 }).sort({ usernameTag: 1 }).lean(),
    Page.find(
      { isPrivate: false },
      { usernameTag: 1, slug: 1, createdAt: 1 }
    )
      .sort({ usernameTag: 1, order_index: 1 })
      .lean(),
  ]);
...
export async function updateUserColours(userId, dashHex, backHex) {
  ...
  return User.findByIdAndUpdate(
    ...
    {
      $set: {
        'dashboard.dashHex': dashHex,
        'dashboard.backHex': backHex,
        'dashboard.themeUpdatedAt': Date.now(),
      },
    },
    { new: true }
  ).lean();
...
export async function updateUserDashboard(userId, changes) {
  await connectDB();
  return User.findByIdAndUpdate(
    userId,
    { $set: infoFieldSet(changes, DASHBOARD_INFO_FIELDS, 'dashboard') },
    { new: true, runValidators: true }
  ).select('dashboard').lean();
}

export async function updateUserTitle(userId, usernameTitle, usernameTag) {
  await connectDB();
  const existing = await User.findById(userId, { usernameTag: 1 }).lean();

  // usernameTag is the account's entire public URL, so the old one is kept and
  // keeps working. Reclaiming a former tag removes it from the history rather
  // than leaving an address that redirects to itself. See LNK-3.
  const update = { $set: { usernameTitle, usernameTag } };
  if (existing && existing.usernameTag !== usernameTag) {
    update.$addToSet = { previousTags: existing.usernameTag };
    update.$pull = { previousTags: usernameTag };
  }
  // $addToSet and $pull cannot touch the same field in one update.
  if (update.$pull) {
    await User.findByIdAndUpdate(userId, { $pull: update.$pull });
    delete update.$pull;
  }
...
/**
 * Resolve a URL's `{usernameTag}` segment, following the rename history.
 *
 * `redirected` is true when the tag reached the account through its history
 * rather than its current address, which is the route handler's signal to
 * issue a permanent redirect to the canonical URL. See LNK-3.
 */
// Memoised per request: the layout resolves the segment to decide the status,
// and the page below reads the same thing to render it. Without cache() that
// is two queries for one answer.
export const resolveUsernameTag = cache(async function resolveUsernameTag(tag) {
  await connectDB();
  const current = await User.findOne({ usernameTag: tag }, PRIVATE_USER_FIELDS).lean();
  if (current) return { user: current, redirected: false };

  const previous = await User.findOne({ previousTags: tag }, PRIVATE_USER_FIELDS).lean();
  if (previous) return { user: previous, redirected: true };

  return { user: null, redirected: false };
});

/** The same, for a page's `{pageSlug}` segment. See LNK-3. */
export const resolvePageSlug = cache(async function resolvePageSlug(userId, slug) {
  await connectDB();
  const current = await Page.findOne({ userId, slug }).lean();
  if (current) return { page: current, redirected: false };

  const previous = await Page.findOne({ userId, previousSlugs: slug }).lean();
  if (previous) return { page: previous, redirected: true };

  return { page: null, redirected: false };
});
...
export async function userOwnsFileUrl(userId, fileUrl) {
  ...
  const ownsPageThumbnail = await Page.exists({ userId, thumbnail: fileUrl });
  if (ownsPageThumbnail) return true;

  // One indexed match now that Post carries userId (CLN-3), rather than
  // fetching every page the user owns to build an $in list.
...
/**
 * A post plus its page, only if the caller owns it.
 *
 * `Post.userId` makes the ownership question a single indexed match rather than
 * "load the post, load its page, compare". The page is still read, because the
 * routes need its usernameTag and slug to revalidate. See CLN-3.
 */
export async function getOwnedPostWithPage(userId, postId) {
  await connectDB();
...
// Memoised per request: on a document load generateMetadata reads the first
// public page for the OG card and the page below reads the same list to render
// it. Identical arguments, one query.
export const getPagesByUser = cache(async function getPagesByUser(userId, includePrivate = false) {
  await connectDB();
  const query = { userId };
...
export async function updatePage(pageId, data) {
  ...
  const allowed = ['title', 'description', 'thumbnail', 'blurDataURL', 'isPrivate', 'isDownloadable', 'slug'];
  const update = {};
  ...
  if (Object.keys(update).length > 0) {
    // Keep the address the page used to have, so links already shared keep
    // working. Reclaiming a former slug drops it from the history rather than
    // leaving an entry that redirects to itself. See LNK-3.
    const previousSlug = update.slug !== undefined ? await Page.findById(pageId, { slug: 1 }).lean() : null;
    const slugChanged = previousSlug && previousSlug.slug !== update.slug;

    if (slugChanged) {
      // $addToSet and $pull cannot touch the same field in one update.
      await Page.findByIdAndUpdate(pageId, { $pull: { previousSlugs: update.slug } });
      return Page.findByIdAndUpdate(
        pageId,
        { $set: update, $addToSet: { previousSlugs: previousSlug.slug } },
        { new: true }
      ).lean();
    }
...
export async function updatePageMeta(pageId, userId, changes) {
  await connectDB();
  return Page.findOneAndUpdate(
    { _id: pageId, userId },
    { $set: infoFieldSet(changes, PAGE_INFO_FIELDS, 'pageMetaData') },
    { new: true, runValidators: true }
  ).select('pageMetaData').lean();
}
...
export async function createPost(pageId, data) {
  ...
  const page = await Page.findById(pageId, { _id: 1, userId: 1 }).lean();
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    try {
      const post = await Post.create({
        pageId,
        // Denormalised from the page, not taken from the caller. See CLN-3.
        userId: page.userId,
        slug,
        title: data.title || '',
        description: data.description || '',
