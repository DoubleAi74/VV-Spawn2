...
for (const [name, fields] of [['dashboard', DASHBOARD_INFO_FIELDS], ['page', PAGE_INFO_FIELDS]]) {
  test(`${name}: returning through a cached route keeps the last edited columns until revalidation`, async t => {
    const env = environment(t, fields);
    env.remote({ gridCols: 4, infoText1: 'Current server text' });
    const cachedRoute = env.data;
    const owner = env.client();
    await settle();
    owner.change('gridCols', 6);
    await owner.save();
    owner.stop();

    const returning = env.client({
      initialValues: cachedRoute,
      initialClientValues: { gridCols: owner.getSnapshot().values.gridCols },
    });
    assert.equal(returning.getSnapshot().values.gridCols, 6, 'first client frame uses the edited count');
    assert.equal(returning.getServerSnapshot().values.gridCols, 4, 'hydration still matches server markup');
    assert.equal(returning.getSnapshot().values.infoText1, cachedRoute.infoText1);
    assert.deepEqual(returning.getSnapshot().pending, [], 'a preview is not an edit');
    await settle();
    assert.equal(returning.getSnapshot().values.gridCols, 6);

    env.remote({ gridCols: 3 });
    await returning.refresh();
    assert.equal(returning.getSnapshot().values.gridCols, 3, 'fresh database values remain authoritative');
    assert.equal(env.writes.length, 1, 'revalidation never republishes a cached count');
  });

  test(`${name}: a remembered automatic layout overrides an old explicit count on return`, t => {
    const env = environment(t, fields);
    const returning = env.client({ initialValues: { gridCols: 6 }, initialClientValues: { gridCols: null } });
    assert.equal(returning.getSnapshot().values.gridCols, null);
    assert.equal(returning.getServerSnapshot().values.gridCols, 6);
    assert.deepEqual(returning.getSnapshot().pending, []);
  });

  test(`${name}: owner layout changes and automatic reset reach open owners and visitors`, async t => {
    const env = environment(t, fields);
    const owner = env.client();
    const other = env.client();
    const visitor = env.client({ canEdit: false });
    await settle();
    other.change('infoText1', 'Draft on another computer');
    owner.change('gridCols', 6);
    await owner.save();
    await other.refresh();
    assert.equal(other.getSnapshot().values.gridCols, 6);
    assert.equal(other.getSnapshot().values.infoText1, 'Draft on another computer');
    assert.deepEqual(other.getSnapshot().pending, ['infoText1']);
    await env.timers.advance(INFO_POLL_MS);
    assert.equal(visitor.getSnapshot().values.gridCols, 6);
    assert.equal(env.client().getSnapshot().values.gridCols, 6, 'new sessions use the stored count');
    visitor.change('gridCols', 2);
    await visitor.save();
    assert.equal(env.data.gridCols, 6, 'visitors cannot alter the published count');
    assert.deepEqual(env.writes[0], { gridCols: 6 });
    owner.change('gridCols', null);
    await env.timers.advance(INFO_POLL_MS);
    assert.equal(other.getSnapshot().values.gridCols, null);
    assert.equal(visitor.getSnapshot().values.gridCols, null);
    assert.equal(env.data.infoText1, 'Draft on another computer');
  });
  ...
  test(`${name}: text, both HTML toggles and clearing a box sync to another open session`, async t => {
    ...
    const visitor = env.client({ canEdit: false });
    await settle();
    for (const { text, mode } of fields.filter(field => field.text)) {
...
test('different pages keep independent published row counts', async t => {
  const firstPage = environment(t, PAGE_INFO_FIELDS);
  const secondPage = environment(t, PAGE_INFO_FIELDS);
  const first = firstPage.client();
  const second = secondPage.client();
  first.change('gridCols', 5);
  second.change('gridCols', 2);
  await Promise.all([first.save(), second.save()]);
  assert.equal(firstPage.data.gridCols, 5);
  assert.equal(secondPage.data.gridCols, 2);
});
