...
export function useInfoSync({ initialValues, initialClientValues, fields, readUrl, writeUrl, canEdit, isEditMode, storageKey }) {
  const restored = useRef(false);
  const wasEditing = useRef(isEditMode);
  const [sync] = useState(() => createInfoSync({
    initialValues,
    initialClientValues: typeof window === 'undefined' ? undefined : initialClientValues?.(),
    fields,
    readUrl,
    writeUrl,
