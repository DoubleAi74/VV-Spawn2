...
test('cssThemeFill never bakes a dummy grey into a loading header', () => {
  assert.equal(cssThemeFill('#570e11', THEME_DASH_CSS), '#570e11');
  assert.equal(cssThemeFill('', THEME_DASH_CSS), THEME_DASH_CSS);
  assert.equal(cssThemeFill(null, THEME_DASH_CSS), THEME_DASH_CSS);
  assert.ok(THEME_DASH_CSS.includes(DEFAULT_DASH_HEX));
});
