...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      ...
      // Keep tag/title aligned with Mongo after renames. `/` and `/login` redirect
      // from the JWT tag; a stale cookie otherwise traps the user on an old slug
      // (and can 404 if previousTags miss), blocking login without a private window.
      if (token.userId) {
        try {
          await connectDB();
          ...
          if (dbUser) {
            token.usernameTag = dbUser.usernameTag;
            token.usernameTitle = dbUser.usernameTitle;
          }
        } catch {
          // Leave token as-is if DB is briefly unavailable.
        }
      }
      return token;
    },
