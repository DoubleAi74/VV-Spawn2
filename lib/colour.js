...
/** Default header / page fills, matching ThemeProvider. */
export const DEFAULT_DASH_HEX = '#2d3e50';
export const DEFAULT_BACK_HEX = '#e5e7eb';

/** Live theme published on :root so a loading header can paint before JS. */
export const THEME_DASH_CSS = `var(--dash-hex, ${DEFAULT_DASH_HEX})`;
export const THEME_BACK_CSS = `var(--back-hex, ${DEFAULT_BACK_HEX})`;

/** Known hex if we have one; otherwise a CSS variable, never a dummy grey. */
export function cssThemeFill(hex, cssVar) {
  return normalizeHex(hex, '') || cssVar;
}
...
export function getInfoPalette(backgroundHex) {
  ...
  if (isDarkBackground) {
    ...
    return {
      ...
      panelBorder: hexToRgba('#ffffff', 0.128),
  ...
  return {
    ...
    panelBorder: 'rgba(163, 163, 163, 0.304)',
