...
// Not pure white and pure black: against a mid-tone both are legible, and the
// softer pair sits better in a design that is otherwise all muted colour.
 ...
 * happen: whichever of the two inks is further from the background wins.
 */
export function readableInkOn(backgroundHex, fallback = FALLBACK_HEX) {
  const safe = normalizeHex(backgroundHex, fallback);
  return contrastRatio(safe, LIGHT_INK) >= contrastRatio(safe, DARK_INK)
    ? LIGHT_INK
    : DARK_INK;
}
