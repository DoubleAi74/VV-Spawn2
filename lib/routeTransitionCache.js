...
/** Loading boundaries can remain mounted while a live view updates its cache. */
export function subscribeRouteSnapshots(listener) {
  listeners.add(listener);
  return () => listeners.delete(listener);
}

function notifySnapshotsChanged() {
...
function persistableInfo(snapshot, keys) {
  ...
  // Text too big for storage still leaves a height worth keeping: it is what
  // lets the live view size its frame straight away instead of publishing
  // data-info-pending and holding the loading cover over ready content.
  if (infoText.length > INFO_SNAPSHOT_MAX_CHARS) {
    return infoHeight ? { [keys.height]: infoHeight } : {};
  }
  return {
    [keys.text]: infoText,
...
export function sanitizeDashboardSnapshot(snapshot) {
  ...
  return {
    ...
    gridCols: normalizePostCols(snapshot.gridCols),
...
/** Colour saves must not leave history snapshots advertising the previous theme. */
export function patchStoredTheme(usernameTag, dashHex, backHex) {
  if (!usernameTag) return;
  ensureHydrated();
  const now = Date.now();
  const dash = getDashboardSnapshot(usernameTag);
  if (dash) {
    dashboardSnapshots.set(usernameTag, { ...dash, dashHex, backHex, updatedAt: now });
    persistMap(dashboardSnapshots, DASH_SNAPSHOT_STORAGE_KEY, sanitizeDashboardSnapshot);
  }
  const prefix = `${usernameTag}/`;
  let pagesChanged = false;
  for (const [key, value] of pageSnapshots.entries()) {
    if (!key.startsWith(prefix)) continue;
    pageSnapshots.set(key, { ...value, dashHex, backHex, updatedAt: now });
    pagesChanged = true;
  }
  if (pagesChanged) {
    persistMap(pageSnapshots, PAGE_SNAPSHOT_STORAGE_KEY, sanitizePageSnapshot);
  }
  if (dash || pagesChanged) notifySnapshotsChanged();
}

/** Save acknowledgements can arrive after the view that queued them unmounts. */
export function updateSnapshotCards(usernameTag, pageSlug, items, isOwner) {
  if (pageSlug) {
    const current = getPageSnapshot(usernameTag, pageSlug);
    if (!current || Boolean(current.isOwner) !== Boolean(isOwner)) return;
    setPageSnapshot(usernameTag, pageSlug, {
      ...current,
      posts: sanitizePageSnapshot({ posts: items }).posts,
    });
  } else {
    ...
    setDashboardSnapshot(usernameTag, {
      ...current,
      pages: sanitizeDashboardSnapshot({
        pages: isOwner ? items : items.filter(item => !item.isPrivate),
      }).pages,
    });
  }
}
