...
import { BLUR_PREVIEW_WIDTH, BLUR_PREVIEW_QUALITY } from './blurPreview.js';
 ...
 * Returns a base64 data URL using the shared preview dimensions and quality.
 */
function generateBlurFromImg(img) {
  const aspect = img.height / img.width;
  const bw = BLUR_PREVIEW_WIDTH;
  const bh = Math.max(1, Math.round(bw * aspect));
  const canvas = document.createElement("canvas");
  canvas.width = bw;
  canvas.height = bh;
  canvas.getContext("2d").drawImage(img, 0, 0, bw, bh);
  return canvas.toDataURL("image/jpeg", BLUR_PREVIEW_QUALITY / 100);
}
 ...
 */
export async function fetchServerBlur(imageUrl, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // 401 / 400 are verdicts, not propagation lag — retrying cannot help.
      if (res.status >= 400 && res.status < 500) {
        console.warn(`fetchServerBlur rejected: ${res.status}`);
        return null;
      }
      console.warn(
    ...
    if (attempt < maxRetries) await delay(attempt * 500);
  }
