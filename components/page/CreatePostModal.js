...
// The visible heading swaps a short and a long form per breakpoint, so it
// cannot serve as the dialog's accessible name — it would read "ImagePost an
// image". These are the names announced instead.
const TAB_LABELS = {
  photo: "Post an image",
  url: "Add a URL link",
  file: "Display a file",
...
export default function CreatePostModal({
  ...
  // A failed create must not re-upload files that already landed.
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      showError("Couldn't create the post", err.message || "The upload did not finish.", {
              ...
              {previewSrc ? (
                <div className="w-16 h-16 rounded-[1px] overflow-hidden border-2 border-emerald-500/40 flex-shrink-0 relative">
                  {isHeicPreview ? (
                    ...
                    <ThumbnailPreview src={previewSrc} />
                  )}
                </div>
              ) : (
                <div
