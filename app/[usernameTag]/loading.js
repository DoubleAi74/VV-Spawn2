    ...
    <div className="p-2 pb-[3px] rounded-[4px] border-[3px] border-neutral-800/20 bg-neutral-200/60 shadow-md h-full">
      ...
      >
        {thumbnail ? (
          ...
          <img src={thumbnail} alt={title || 'Page preview'} className="absolute inset-0 w-full h-full object-cover" />
        ) : (
          <>
      ...
      <div className="flex pl-1 pr-1 items-center justify-between gap-1 h-8 w-full overflow-hidden">
        {title ? (
          <h3 className="min-w-0 truncate font-bold text-neutral-800/90 text-sm" title={title}>
            {title}
          </h3>
        ) : (
...
export default function DashboardLoading() {
  ...
  const usernameTag = typeof params?.usernameTag === 'string' ? params.usernameTag : '';
  const snapshot = getDashboardSnapshot(usernameTag);
  const hasSnapshotCards = Array.isArray(snapshot?.pages) && snapshot.pages.length > 0;

  const dashHex = normalizeHex(snapshot?.dashHex, '#6b7280');
  const backHex = normalizeHex(snapshot?.backHex, '#d4d4d8');
  const pages = hasSnapshotCards ? snapshot.pages : [];

  return (
    <div className="min-h-[150vh] overscroll-none" style={{ backgroundColor: backHex }}>
      <header
      ...
      >
        <div className="w-full px-0">
          <div className="flex items-center min-h-[70px] sm:min-h-[90px] px-6 sm:px-8">
            {snapshot?.usernameTitle ? (
              <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight truncate" style={{ color: lighten(dashHex, 245) }}>
                {snapshot.usernameTitle}
              </h1>
            ) : (
        ...
        <div className="w-full pb-[5px]" style={{ backgroundColor: dashHex }}>
          <div className="h-[8px] w-full border-t border-black/15" style={{ backgroundColor: lighten(dashHex, 30) }} />
        </div>
      </header>

      <main className="w-full px-[10px] md:px-8 pt-[1.6rem] pb-72">
        {pages.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-[7px] sm:gap-4">
            {pages.map((page, index) => (
              <DashboardLoadingCard key={page._id || `skeleton-${index}`} page={page} />
            ))}
          </div>
        )}
      </main>
