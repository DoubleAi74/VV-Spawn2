    ...
    <div className="w-full p-1 rounded-[2px] bg-neutral-200/60 shadow-lg border-[3px] border-neutral-800/20 h-full flex flex-col">
      ...
      >
        {thumbnail ? (
          ...
          <img src={thumbnail} alt={title || 'Post preview'} className="absolute inset-0 w-full h-full object-cover" />
        ) : (
          <>
      ...
      <div className="px-1 pt-[4px] truncate text-xs font-bold text-neutral-800/85">
        {title || <span className="inline-block h-3 w-3/5 rounded-[2px] bg-neutral-800/10 align-middle" />}
      </div>
    </div>
  );
}
...
export default function PageViewLoading() {
  ...
  const posts = snapshot?.posts?.length
    ? snapshot.posts
    : [];
          ...
          {posts.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-[7px] sm:gap-4">
              {posts.map((post, index) => (
                <PostLoadingCard key={post._id || `skeleton-${index}`} post={post} />
              ))}
            </div>
          )}
        </div>
